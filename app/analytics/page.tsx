...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    let mounted = true
      ; (async () => {
        const endpoints = [
          { key: "overview", path: "/api/analytics/overview" },
          { key: "portfolio", path: "/api/analytics/portfolio" },
          { key: "demographics", path: "/api/analytics/demographics?range=365" },
          { key: "repayments", path: "/api/analytics/repayments?range=30" },
          { key: "performance", path: "/api/analytics/loan-performance?vintageMonths=12" },
          { key: "officers", path: "/api/analytics/officers?range=30" },
          { key: "risk", path: "/api/analytics/risk?range=365" },
        ]

        const results = await Promise.allSettled(
          endpoints.map((e) =>
            apiGet(e.path)
              .then((body) => ({ key: e.key, ok: true, body }))
              .catch((err) => ({ key: e.key, ok: false, error: err.message }))
          )
        )
        ...
        for (const r of results) {
          if (r.status === "fulfilled") {
            const v: any = r.value
            if (!v.ok) {
              nextErrors[v.key] = v.error || `endpoint returned error`
              continue
            }
            switch (v.key) {
              case "overview":
                        ...
                        {demographics?.ageBuckets
                          ? Object.entries(demographics.ageBuckets).map(([k, v]) => (
                            <div key={k} className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-bold">{v as string}</span>
                            </div>
                          ))
                        ...
                        {repayments?.byMethod ? (
                          Object.entries(repayments.byMethod).map(([m, cents]) => (
                            <div key={m} className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-bold">{centsToKES(cents as number)}</span>
                            </div>
                          ))
                        ) : (
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="p-4 neumorphic-inset rounded-xl">
                      ...
                      <p className="text-2xl font-bold">{risk?.topRiskDrivers ? Math.round(((risk.topRiskDrivers.find((d: any) => d.driver === 'lowSavings')?.impact as number) || 0) * 100) : "—"} clients</p>
